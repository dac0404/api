...
module.exports = function(server, db_config){
	...
	server.post("/ApplyFilterClientImm/",(req, res, next) =>{
		var query = "call CreateFilterImm(?,?,?,?,?)";
		conectionDB();

		cnn.query(query, [req.body.FillStatus, req.body.FilterUsualTP, req.body.SexImmigServ, 
			req.body.CreatedFrom, req.body.RowIndex], function(err, result, fields){
			if (err){
	          res.send(500, {message: err});
	          cnn.end();
	          return next(false);
...
function conectionDB(){
    ...
    cnn.on('error', function(err) {
      console.log('db error immigration', err.code);
      if(err.code === 'PROTOCOL_CONNECTION_LOST') {
          //conectionDB();
          console.log("el erro immigration");
        } else {
          throw err;
        }
    });                               
