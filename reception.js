...
module.exports = function(server, db_config){
	...
	server.put('/save_ProcRec/', (req,res,next) =>{
		...
		cnn.query(query, [req.body.IdClient,req.body.proc1,req.body.proc2, req.body.proc3,
			...
			req.body.proc40, req.body.proc41,req.body.proc42,req.body.proc43,
			req.body.proc44,req.body.proc45,req.body.proc46,req.body.proc47,req.body.proc48,
			req.body.proc49,req.body.proc50,req.body.proc51,req.body.proc52,req.body.proc53,
			req.body.TaxYear],function(err, result, fields){
			if (err){
	          res.send(500, {message: err});
	          cnn.end();
	          return next(false);
	...
	server.post('/CreateNewTicket/', (req, res, next) =>{
		...
		cnn.query(query, [req.body.wSSN,req.body.NameClientTicket,req.body.SelectAgent,req.body.TypeService,req.body.ReasonVisit,1], function(err, result, fields){
	        ...
	        if(result != undefined && result[0] != undefined){
	          res.json({success:true , hasdata: true , data: result[2]});
	          cnn.end();
	          return next();
	        }else{
	          //missin parameter
	...
	server.post('/reportticket', (req, res, next) =>{
		...
		cnn.query(query,[req.body.wTicket], function(err, result, fields){
	        ...
	        if(result != undefined && result[0] != undefined){
	          ...
	          UpdateTicketXML(req.body.wPC, req.body.wTicket);
	...
	function UpdateTicketXML(wPC, IdTicket){
		var query = "UPDATE tickets SET StringXML = ?, PcPrinted = ? WHERE IdTicket = ? ";
		conectionDB();
		cnn.query(query,[filexml, wPC, IdTicket], function(err, result, fields){
			// if (err){
	...
	function conectionDB(){
	    ...
	    cnn.on('error', function(err) {
	      console.log('db error reception', err.code);
	    });                               
	  }
