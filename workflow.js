...
module.exports = function(server, db_config) {
	...
	server.post("/UpdateBinDataEntry",(req,res,next)=>{
		conectionDB();
		var query = "UPDATE tickets SET IdEmpDataEntry = ? WHERE IdTicket = ? limit 1";
	...
	server.post('/CheckClientOver24h',(req, res, next)=>{
		var query = "call CheckClientOver24h(?)";

		conectionDB();
		cnn.query(query, [req.body.IdPrep], function(err, result, fields){
			if (err){
				res.send(500,{message:err});
				cnn.end();
				return;
	...
	server.post('/FilterClientOver24h',(req, res, next)=>{
		var query = "call FilterClientsOver24h(?)";
	...
	function conectionDB(){
	    ...
	    cnn.on('error', function(err) {
	      console.log('db error workflow', err.code);
	    });                               
	  }
